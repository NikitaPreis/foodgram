...
def get_recipe_in_favorite_list(user, recipe):
    return FavoriteList.objects.filter(user=user, recipe=recipe)


def get_recipes_from_favorite_list(recipes, user):
    return recipes.filter(favorite_recipe__user=user)


def add_recipe_to_favorite_list(user, recipe):
    return FavoriteList.objects.create(user=user, recipe=recipe)
